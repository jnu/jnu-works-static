...
// BUG -- THIS does not work when <base/> tag is present.
...
$.widget( "ui.tabs", {
	...
	_sanitizeSelector: function( hash ) {
		// MODIFIED jn 11/11/2012 TO ACCOMODATE <BASE/>
		//hashp = hash.split('#');
		//_hash = '#' + hashp[hashp.length-1];
		// END JN MODIFIED
	...
	_processTabs: function() {
		...
		this.anchors.each(function( i, anchor ) {
			...
			// inline tab
			// BUG -- if <BASE/> tag is present, anchors are not treated as local!
			if ( isLocal( anchor ) ) {
				selector = anchor.hash;
				panel = that.element.find( that._sanitizeSelector( selector ) );
			// remote tab
			} else {
				console.error("IS NOT LOCAL");
