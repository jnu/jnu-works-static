...
var numPinned=pinnedObjects.length;for(i=0;i<numPinned;i++){var pinObj=pinnedObjects[i];var el=pinObj.el;if(pinObj.state!=='PINNED'){if(pinObj.state==='UPDATE'){resetPinObj(pinObj);}
startPoint=superscrollorama.settings.isVertical?pinObj.spacer.offset().top+scrollContainerOffset.y:pinObj.spacer.offset().left+scrollContainerOffset.x;startPoint+=pinObj.offset;endPoint=startPoint+pinObj.dur;var jumpedPast=((currScrollPoint>endPoint&&pinObj.state==='BEFORE')||(currScrollPoint<startPoint&&pinObj.state==='AFTER'));var inPinAra=(currScrollPoint>startPoint&&currScrollPoint<endPoint);if(inPinAra||jumpedPast){if(pinObj.pushFollowers&&el.css('position')==="static"){el.css('position',"relative");}
pinObj.origPositioning={pos:el.css('position'),top:pinObj.spacer.css('top'),left:pinObj.spacer.css('left')};pinObj.fixedPositioning={top:superscrollorama.settings.isVertical?-pinObj.offset:pinObj.spacer.offset().top,left:superscrollorama.settings.isVertical?pinObj.spacer.offset().left:-pinObj.offset};el.css('position','fixed');el.css('top',pinObj.fixedPositioning.top);el.css('left',pinObj.fixedPositioning.left);pinObj.pinStart=startPoint;pinObj.pinEnd=endPoint;if(pinObj.pushFollowers){if(superscrollorama.settings.isVertical){pinObj.spacer.height(pinObj.dur+el.outerHeight());}else{pinObj.spacer.width(pinObj.dur+el.outerWidth());}}else{if(pinObj.origPositioning.pos==="absolute"){pinObj.spacer.width(0);pinObj.spacer.height(0);}else{if(superscrollorama.settings.isVertical){pinObj.spacer.height(el.outerHeight());}else{pinObj.spacer.width(el.outerWidth());}}}
