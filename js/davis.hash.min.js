 ...
 * MIT Licensed
 */
Davis.hashRouting=function(options){if(!options)
options={};if(typeof(options.forceHashRouting)=='undefined')
options.forceHashRouting=false;if(typeof(options.prefix)=='undefined')
options.prefix='';if(typeof(options.normalizeInitialLocation)=='undefined')
options.normalizeInitialLocation=false;if(typeof(options.pollerInterval)=='undefined')
options.pollerInterval=100;if(typeof(options.forcePolling)=='undefined')
options.forcePolling=false;if(typeof(options.location)=='undefined')
options.location=window.location;var callbacks=[];var bindHashChange=function(){if("onhashchange"in window&&!options.forcePolling){Davis.$(window).bind('hashchange',checkForLocationChange);}else{setTimeout(locationPoller,options.pollerInterval);}};var invokeCallbacks=function(request){Davis.utils.forEach(callbacks,function(callback){callback(request);});};var onChange=function(handler){callbacks.push(handler);};var hashLocationPattern=new RegExp("#"+options.prefix+"(.*)$")
var current=function(){var match=options.location.hash.match(hashLocationPattern);if(match)
return match[1];else
return'/';};var onHashChange=function(){var path=current();if(path){invokeCallbacks(new Davis.Request({fullPath:path,method:"get"}));}};var normalize=function(usingHashRouting,hashLocation,normalLocation){if(hashLocation&&hashLocation!='/'){if(usingHashRouting){if(normalLocation!='/'){return"/#"+options.prefix+hashLocation;}}else{return hashLocation;}}else{if(usingHashRouting&&normalLocation!='/'){return"/#"+options.prefix+normalLocation;}}
return null;};getLocation=function(){return options.location.hash;};var lastPolledLocation=getLocation();var checkForLocationChange=function(){if(lastPolledLocation!=getLocation()){lastPolledLocation=getLocation();onHashChange();}};var locationPoller=function(){checkForLocationChange();setTimeout(locationPoller,options.pollerInterval);};var wrapper=function(request,opts,setter){setter("#"+options.prefix+request.location());lastPolledLocation=getLocation();if(opts&&opts.silent)return
invokeCallbacks(request);};var assign=function(request,opts){wrapper(request,(opts||false),function(string){options.location.assign(string);});};var replace=function(request,opts){wrapper(request,(opts||false),function(string){options.location.replace(string);});};return function(Davis){var usingHashRouting=!Davis.supported()||options.forceHashRouting;if(options.normalizeInitialLocation){normalizedLocation=normalize(usingHashRouting,current(),options.location.pathname);if(normalizedLocation){options.location.replace(normalizedLocation);}}
if(!usingHashRouting)
return;Davis.supported=function(){return true;}
Davis.location.setLocationDelegate({assign:assign,current:current,replace:replace,onChange:onChange});bindHashChange();}};

// extend Davis with hashRouting
